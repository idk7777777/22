    ...
    if wrapper_config.PROXY and wrapper_config.PROXY_USERNAME  and wrapper_config.PROXY_PASSWORD:
        process = Popen(
            [
                ...
                '--dump-format=CSV',
                '--answers=quit=n,crack=n,new_xp_cmdshell=N,Are you sure of your choice (breaking warranty)=Y,Do you want to reduce the number of requests=Y,do you want to store hashes to a temporary=N,random integer value for option=Y,how do you want to proceed=C,Do you want to try with a random integer=Y,while server wants to set its own=Y,Do you want to keep testing the=Y,do you want to perform a dictionary-based attack=N,optimize=Y,provided=1,which=a',
                '--search',
                '-C %s' % dump,
